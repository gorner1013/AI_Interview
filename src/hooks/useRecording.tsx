...
export const useRecording = (
  ...
  const uploadVideo = async (url, blob) => {
    ...
    let response;
    try {
      response = await fetch(`${process.env.HTTP_BASE_URL}/interview_videos`, {
        method: "POST",
        body: formData,
      });
    } catch (error) {
...
const mergeAudioStreams = (desktopStream, voiceStream) => {
	...
	const desktopSource = context.createMediaStreamSource(desktopStream);
	const desktopGain = context.createGain();
	desktopGain.gain.value = 0.3; // Adjust gain
	desktopSource.connect(desktopGain).connect(destination);
  
	const voiceSource = context.createMediaStreamSource(voiceStream);
	const voiceGain = context.createGain();
	voiceGain.gain.value = 0.7; // Adjust gain
	voiceSource.connect(voiceGain).connect(destination);
  
	// You want to return a MediaStream, not AudioTracks
  ...
  const startRecording = async () => {
	...
	// mimeTypeを設定
	const stream = new MediaStream([...audioStream.getAudioTracks(),  ...displayStream.getVideoTracks()]);
